          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
              <div className="p-5 border-b dark:border-gray-700 flex justify-between items-center sticky top-0 bg-white dark:bg-gray-800">
                ...
                <button onClick={() => { setShowNotaFiscalModal(false); setEditingNota(null); }} className="p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700">
                  <XCircleIcon className="w-5 h-5" />
                </button>
              </div>
              <div className="p-5 space-y-6">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <label className="block text-sm font-medium mb-1 dark:text-gray-300">Nº Nota</label>
                    <input type="text" value={notaFiscal.numero_nota} onChange={(e) => setNotaFiscal({...notaFiscal, numero_nota: e.target.value})}
                      className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-1 dark:text-gray-300">Nº Lote</label>
                    <input type="text" value={notaFiscal.numero_lote} onChange={(e) => setNotaFiscal({...notaFiscal, numero_lote: e.target.value})}
                      className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-1 dark:text-gray-300">Competência</label>
                    <input type="month" value={notaFiscal.competencia} onChange={(e) => setNotaFiscal({...notaFiscal, competencia: e.target.value})}
                      className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-1 dark:text-gray-300">Data Emissão</label>
                    <input type="date" value={notaFiscal.data_emissao} onChange={(e) => setNotaFiscal({...notaFiscal, data_emissao: e.target.value})}
                      className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-1 dark:text-gray-300">Previsão Pagamento</label>
                    <input type="date" value={notaFiscal.data_previsao_pagamento} onChange={(e) => setNotaFiscal({...notaFiscal, data_previsao_pagamento: e.target.value})}
                      className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-1 dark:text-gray-300">Status</label>
                    <select value={notaFiscal.status} onChange={(e) => setNotaFiscal({...notaFiscal, status: e.target.value})}
                      className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                ...
                {/* Valores e Impostos */}
                <div>
                  <h4 className="font-medium text-sm text-gray-700 dark:text-gray-300 mb-3">Impostos e Deduções</h4>
                  <div className="grid grid-cols-4 gap-3">
                    <div>
                      <label className="block text-xs text-gray-500 mb-1">Base Cálculo</label>
                      <input type="number" step="0.01" value={notaFiscal.base_calculo} onChange={(e) => atualizarNotaFiscal('base_calculo', e.target.value)}
                        className="w-full border rounded-lg px-2 py-1 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                    </div>
                    <div>
                      <label className="block text-xs text-gray-500 mb-1">ISS (%)</label>
                      <input type="number" step="0.01" value={notaFiscal.aliquota_iss} onChange={(e) => atualizarNotaFiscal('aliquota_iss', e.target.value)}
                        className="w-full border rounded-lg px-2 py-1 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                    </div>
                    <div>
                      <label className="block text-xs text-gray-500 mb-1">IR (%)</label>
                      <input type="number" step="0.01" value={notaFiscal.aliquota_ir} onChange={(e) => atualizarNotaFiscal('aliquota_ir', e.target.value)}
                        className="w-full border rounded-lg px-2 py-1 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                    </div>
                    <div>
                      <label className="block text-xs text-gray-500 mb-1">CSLL (%)</label>
                      <input type="number" step="0.01" value={notaFiscal.aliquota_csll} onChange={(e) => atualizarNotaFiscal('aliquota_csll', e.target.value)}
                        className="w-full border rounded-lg px-2 py-1 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                    </div>
                    <div>
                      <label className="block text-xs text-gray-500 mb-1">PIS (%)</label>
                      <input type="number" step="0.01" value={notaFiscal.aliquota_pis} onChange={(e) => atualizarNotaFiscal('aliquota_pis', e.target.value)}
                        className="w-full border rounded-lg px-2 py-1 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                    </div>
                    <div>
                      <label className="block text-xs text-gray-500 mb-1">COFINS (%)</label>
                      <input type="number" step="0.01" value={notaFiscal.aliquota_cofins} onChange={(e) => atualizarNotaFiscal('aliquota_cofins', e.target.value)}
                        className="w-full border rounded-lg px-2 py-1 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                    </div>
                    <div>
                      <label className="block text-xs text-gray-500 mb-1">IBS (%)</label>
                      <input type="number" step="0.01" value={notaFiscal.aliquota_ibs} onChange={(e) => atualizarNotaFiscal('aliquota_ibs', e.target.value)}
                        className="w-full border rounded-lg px-2 py-1 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                    </div>
                    <div>
                      <label className="block text-xs text-gray-500 mb-1">CBS (%)</label>
                      <input type="number" step="0.01" value={notaFiscal.aliquota_cbs} onChange={(e) => atualizarNotaFiscal('aliquota_cbs', e.target.value)}
                        className="w-full border rounded-lg px-2 py-1 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-4 gap-3 mt-3">
                    <div>
                      <label className="block text-xs text-gray-500">Valor Total</label>
                      <input type="text" value={formatCurrency(notaFiscal.valor_total)} disabled
                        className="w-full bg-gray-100 dark:bg-gray-600 border rounded-lg px-2 py-1 text-sm font-semibold" />
                    </div>
                    <div>
                      <label className="block text-xs text-gray-500">Valor ISS</label>
                      <input type="text" value={formatCurrency(notaFiscal.valor_iss)} disabled
                        className="w-full bg-gray-100 dark:bg-gray-600 border rounded-lg px-2 py-1 text-sm" />
                    </div>
                    <div>
                      <label className="block text-xs text-gray-500">Valor IR</label>
                      <input type="text" value={formatCurrency(notaFiscal.valor_ir)} disabled
                        className="w-full bg-gray-100 dark:bg-gray-600 border rounded-lg px-2 py-1 text-sm" />
                    </div>
                    <div>
                      <label className="block text-xs text-gray-500">Valor Líquido</label>
                      <input type="text" value={formatCurrency(notaFiscal.valor_liquido)} disabled
                        className="w-full bg-green-50 dark:bg-green-900/20 border border-green-200 rounded-lg px-2 py-1 text-sm font-bold text-green-700" />
                    </div>
                  </div>
                </div>
                ...
                {/* Anexos */}
                <div>
                  <h4 className="font-medium text-sm text-gray-700 dark:text-gray-300 mb-3">Anexos</h4>
                  <div className="flex gap-2 mb-3">
                    <input type="text" placeholder="Nome do arquivo" value={novoAnexoNome} onChange={(e) => setNovoAnexoNome(e.target.value)}
                      className="flex-1 border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                    <input type="text" placeholder="URL ou caminho" value={novoAnexoUrl} onChange={(e) => setNovoAnexoUrl(e.target.value)}
                      className="flex-1 border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                    <button onClick={addAnexo} className="bg-blue-600 text-white px-3 py-2 rounded-lg text-sm">+</button>
                  </div>
                  {anexos.length > 0 && (
                    <div className="space-y-2">
                      {anexos.map((anexo, i) => (
                        <div key={i} className="flex items-center justify-between bg-gray-50 dark:bg-gray-700/50 p-2 rounded-lg">
                          <div className="flex items-center gap-2">
                            <DocumentTextIcon className="w-4 h-4 text-blue-500" />
                            <span className="text-sm">{anexo.nome}</span>
                            <span className="text-xs text-gray-500">{anexo.data}</span>
                          </div>
                          <button onClick={() => removeAnexo(i)} className="text-red-500 hover:text-red-700">
                ...
                <div>
                  <label className="block text-sm font-medium mb-1 dark:text-gray-300">Observações</label>
                  <textarea rows="3" value={notaFiscal.observacoes} onChange={(e) => setNotaFiscal({...notaFiscal, observacoes: e.target.value})}
                    className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                </div>
              </div>
              <div className="p-5 border-t dark:border-gray-700 flex justify-end gap-3 sticky bottom-0 bg-white dark:bg-gray-800">
                <button onClick={() => { setShowNotaFiscalModal(false); setEditingNota(null); }} className="px-4 py-2 border rounded-lg text-sm">Cancelar</button>
                <button onClick={handleSaveNotaFiscal} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700">
