...
export default function OperacaoHospitalar({ patientId: contextPatientId = '' }) {
  ...
  const internacoesAtivas = internacoes.filter(item => item.status === 'ativa');
  const pacientesDisponiveis = pacientes.filter(patient => !internacoesAtivas.some(admission => String(admission.paciente_id) === String(patient.id)));

  const internar = async event => {
    event.preventDefault();
    if (!admission.paciente_id || !admission.leito_id) return toast.error('Selecione o paciente e o leito disponível.');
    try {
  ...
  return <div className="min-h-screen bg-gray-50 p-4 dark:bg-gray-900 md:p-6">
    <div className="mx-auto max-w-7xl">
      ...
      {tab==='internacoes' && <><div className="mb-4 flex items-center justify-between"><p className="text-sm text-gray-500">Exibindo somente pacientes atualmente internados.</p><button onClick={()=>setShowAdmission(true)} className="flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2 text-sm font-medium text-white"><PlusIcon className="h-4 w-4"/>Nova internação</button></div><div className="overflow-hidden rounded-2xl border bg-white dark:border-gray-700 dark:bg-gray-800"><div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-gray-50 text-left text-xs uppercase text-gray-500 dark:bg-gray-700"><tr><th className="p-3">Internação</th><th className="p-3">Paciente</th><th className="p-3">Leito</th><th className="p-3">Médico</th><th className="p-3">Entrada</th><th className="p-3">Status</th><th className="p-3">Ação</th></tr></thead><tbody>{internacoesAtivas.map(i=><tr key={i.id} className="border-t dark:border-gray-700"><td className="p-3 font-mono">{i.numero_internacao}</td><td className="p-3 font-medium dark:text-white">{i.pacientes?.nome||'-'}</td><td className="p-3">{i.leitos?.codigo||'-'}</td><td className="p-3">{i.prestadores?.nome||'-'}</td><td className="p-3">{new Date(i.data_entrada).toLocaleString('pt-BR')}</td><td className="p-3"><span className="rounded-full bg-blue-50 px-2 py-1 text-xs text-blue-700">Internado</span></td><td className="p-3"><button onClick={()=>registrarAlta(i)} className="rounded-lg bg-emerald-50 px-3 py-1 text-xs font-medium text-emerald-700">Registrar alta</button></td></tr>)}</tbody></table>{!internacoesAtivas.length&&<p className="p-10 text-center text-sm text-gray-500">Nenhum paciente internado nesta unidade.</p>}</div></div></>}
