...
export default function Atendimentos() {
  ...
  const [buscaProfissionalNome, setBuscaProfissionalNome] = useState('');
  ...
  const buscarProfissionalPorNome = useCallback(async (nome) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        // Mostrar lista de profissionais encontrados
        const profissional = data[0];
        setFormData(prev => ({
          ...prev,
          profissional_solicitante: profissional.nome,
          conselho_solicitante: profissional.codigo_conselho_ans || '06',
          numero_conselho_solicitante: profissional.numero_conselho,
          uf_solicitante: profissional.uf_conselho || '35',
          cbos_solicitante: profissional.cbos || '225125'
        }));
        toast.success(`Profissional ${profissional.nome} encontrado!`);
      }
    } catch (error) {
  ...
  // Buscar profissional executante por nome
  const buscarProfissionalExecutantePorNome = useCallback(async (nome) => {
    ...
    try {
      ...
      if (data && data.length > 0 && data[0]) {
        ...
        toast.success(`Profissional ${prestador.nome} selecionado!`);
      }
    } catch (error) {
      console.error('Erro ao buscar profissional:', error);
    }
  ...
  const handleAdicionarItem = () => {
    ...
    const novoItem = {
      ...
      hora_inicial: currentItem.hora_inicial || '',
      hora_final: currentItem.hora_final || ''
    };
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
          <div className="bg-white dark:bg-gray-800 rounded-xl border p-4">
            <div className="flex justify-between">
              <div><p className="text-xs text-gray-500">Total de Guias</p><p className="text-2xl font-bold">{atendimentos.length}</p></div>
              <DocumentPlusIcon className="w-8 h-8 text-blue-500 opacity-50" />
            </div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-xl border p-4">
            <div className="flex justify-between">
              <div><p className="text-xs text-gray-500">Pendentes</p><p className="text-2xl font-bold text-yellow-600">{pendentes}</p></div>
              <ClockIcon className="w-8 h-8 text-yellow-500 opacity-50" />
            </div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-xl border p-4">
            <div className="flex justify-between">
              <div><p className="text-xs text-gray-500">Autorizados</p><p className="text-2xl font-bold text-blue-600">{autorizados}</p></div>
              <CheckIcon className="w-8 h-8 text-blue-500 opacity-50" />
            </div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-xl border p-4">
            <div className="flex justify-between">
              <div><p className="text-xs text-gray-500">Autorização Parcial</p><p className="text-2xl font-bold text-orange-600">{parciais}</p></div>
              <ExclamationTriangleIcon className="w-8 h-8 text-orange-500 opacity-50" />
            </div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-xl border p-4">
            <div className="flex justify-between">
              <div><p className="text-xs text-gray-500">Valor Pendente</p><p className="text-2xl font-bold text-purple-600">R$ {valorTotalPendente.toFixed(2)}</p></div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border p-4 mb-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="relative">
              ...
              <input type="text" placeholder="Buscar por paciente, carteira ou guia..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-white dark:bg-gray-700 border rounded-lg pl-8 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <select value={filtroStatus} onChange={(e) => setFiltroStatus(e.target.value)} className="bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm">
              <option value="todos">Todos os status</option>
              {STATUS_ATENDIMENTO.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}
            </select>
            <select value={filtroConvenio} onChange={(e) => setFiltroConvenio(e.target.value)} className="bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm">
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 dark:bg-gray-700/50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500">Convênio</th>
                  <th className="px-4 py-3 text-center text-xs font-medium text-gray-500">Itens</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500">Valor Total</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500">Status</th>
                  <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 w-32">Ações</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {atendimentosFiltrados.map((a) => (
                  <tr key={a.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    <td className="px-4 py-3 text-sm text-gray-500">{a.data_atendimento ? format(new Date(a.data_atendimento), 'dd/MM/yyyy') : (a.itens && a.itens[0] ? format(new Date(a.itens[0].data_execucao), 'dd/MM/yyyy') : '-')}</td>
                    <td className="px-4 py-3 text-sm font-mono text-gray-600">{a.numero_guia_prestador}</td>
                    <td className="px-4 py-3 text-sm text-gray-800">{a.paciente_nome}</td>
                    <td className="px-4 py-3 text-sm font-mono text-gray-600">{a.numero_carteira}</td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${a.paciente_convenio_nome && a.paciente_convenio_nome !== 'Sem convênio' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                    ...
                    <td className="px-4 py-3 text-sm font-semibold text-gray-700">R$ {a.valor_total?.toFixed(2)}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-center">
                      <div className="flex gap-1 justify-center">
                        <button onClick={() => handleViewItens(a)} className="p-1 rounded-lg text-gray-600 hover:bg-gray-100" title="Ver Itens"><EyeIcon className="w-4 h-4" /></button>
                        <button onClick={() => handleEdit(a)} className="p-1 rounded-lg text-blue-600 hover:bg-blue-50" title="Editar"><PencilIcon className="w-4 h-4" /></button>
                        <button onClick={() => handleDelete(a.id)} className="p-1 rounded-lg text-red-600 hover:bg-red-50" title="Excluir"><TrashIcon className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
                {atendimentosFiltrados.length === 0 && (
                  <tr><td colSpan="9" className="px-4 py-12 text-center text-gray-500"><DocumentPlusIcon className="w-12 h-12 mx-auto mb-3 opacity-50" />Nenhum atendimento encontrado</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        {showItensModal && selectedGuia && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-5xl max-h-[80vh] overflow-y-auto">
              <div className="sticky top-0 bg-white dark:bg-gray-800 border-b p-5">
                <div className="flex justify-between">
                  <h3 className="text-xl font-semibold">Itens da Guia</h3>
                  <button onClick={() => setShowItensModal(false)} className="p-2 rounded-lg hover:bg-gray-100"><XMarkIcon className="w-5 h-5 text-gray-500" /></button>
                </div>
              </div>
              <div className="p-5">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5 p-4 bg-gray-50 rounded-xl">
                  <div><span className="text-xs text-gray-500">Guia:</span> <span className="text-sm font-mono font-medium">{selectedGuia.numero_guia_prestador}</span></div>
                  <div><span className="text-xs text-gray-500">Paciente:</span> <span className="text-sm font-medium">{selectedGuia.paciente_nome}</span></div>
                  <div><span className="text-xs text-gray-500">Carteira:</span> <span className="text-sm font-mono">{selectedGuia.numero_carteira}</span></div>
                  <div><span className="text-xs text-gray-500">Convênio:</span> <span className="text-sm font-medium text-blue-600">{selectedGuia.paciente_convenio_nome || '-'}</span></div>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="px-3 py-2 text-left text-xs">Seq</th>
                        <th className="px-3 py-2 text-left text-xs">Data</th>
                        <th className="px-3 py-2 text-left text-xs">H.I</th>
                        <th className="px-3 py-2 text-left text-xs">H.F</th>
                    ...
                    <tbody className="divide-y">
                      {(selectedGuia.itens || []).map((item, idx) => (
                        <tr key={idx} className="hover:bg-gray-50">
                          <td className="px-3 py-2 text-xs text-center">{idx + 1}</td>
                    ...
                    <tfoot className="bg-gray-50">
                      <tr className="border-t"><td colSpan="8" className="px-3 py-2 text-right font-semibold">Total da Guia:</td><td colSpan="2" className="px-3 py-2 text-right font-bold text-blue-600">R$ {selectedGuia.valor_total?.toFixed(2)}</td></tr>
                    </tfoot>
                  </table>
                </div>
                <div className="flex justify-end mt-5 pt-4 border-t">
                  <button onClick={() => setShowItensModal(false)} className="px-4 py-2 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-lg text-sm font-medium shadow-md">Fechar</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {showModal && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-6xl max-h-[90vh] overflow-y-auto">
              <div className="sticky top-0 bg-white dark:bg-gray-800 border-b p-5 z-10">
                <div className="flex justify-between">
                  <h3 className="text-xl font-semibold">{editing ? 'Editar Guia' : 'Nova Guia'}</h3>
                  <button onClick={resetModal} className="p-2 rounded-lg hover:bg-gray-100"><XMarkIcon className="w-5 h-5 text-gray-500" /></button>
                </div>
              </div>
              <div className="p-5">
                {/* Tabs */}
                <div className="flex flex-wrap gap-1 border-b mb-5">
                  <button onClick={() => setAba('paciente')} className={`px-4 py-2 text-sm font-medium rounded-t-lg ${aba === 'paciente' ? 'bg-blue-50 text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700'}`}><UserGroupIcon className="w-4 h-4 inline mr-1" /> Paciente</button>
                  <button onClick={() => setAba('autorizacao')} className={`px-4 py-2 text-sm font-medium rounded-t-lg ${aba === 'autorizacao' ? 'bg-blue-50 text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700'}`}><CheckIcon className="w-4 h-4 inline mr-1" /> Autorização</button>
                  <button onClick={() => setAba('solicitante')} className={`px-4 py-2 text-sm font-medium rounded-t-lg ${aba === 'solicitante' ? 'bg-blue-50 text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700'}`}><IdentificationIcon className="w-4 h-4 inline mr-1" /> Solicitante</button>
                  <button onClick={() => setAba('atendimento')} className={`px-4 py-2 text-sm font-medium rounded-t-lg ${aba === 'atendimento' ? 'bg-blue-50 text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700'}`}><CalendarIcon className="w-4 h-4 inline mr-1" /> Atendimento</button>
                  <button onClick={() => setAba('procedimentos')} className={`px-4 py-2 text-sm font-medium rounded-t-lg ${aba === 'procedimentos' ? 'bg-blue-50 text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700'}`}><BeakerIcon className="w-4 h-4 inline mr-1" /> Procedimentos</button>
                </div>
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  {aba === 'paciente' && (
                    <div className="space-y-4">
                      <div><label className="block text-sm font-medium mb-1">Paciente *</label>
                        ...
                        {formData.paciente_carteira && <div className="mt-2 p-3 bg-blue-50 rounded-lg"><p className="text-xs text-blue-700"><strong>Carteira:</strong> {formData.paciente_carteira} | <strong>Convênio:</strong> {formData.convenio_nome || 'Não definido'}</p></div>}
                      </div>
                      <div><label className="block text-sm font-medium mb-1">Observações</label><textarea rows="3" value={formData.observacao} onChange={e => setFormData({...formData, observacao: e.target.value})} className="w-full bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm" placeholder="Informações adicionais sobre o atendimento..." /></div>
                      {editing && (<div><label className="block text-sm font-medium mb-1">Status</label><select value={formData.status} onChange={e => setFormData({...formData, status: e.target.value})} className="w-full bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm">{STATUS_ATENDIMENTO.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}</select></div>)}
                    </div>
                  )}
                  ...
                  {aba === 'solicitante' && (
                    <div className="space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div><label className="block text-sm font-medium mb-1">Código na Operadora</label><input type="text" value={formData.codigo_operadora} onChange={e => setFormData({...formData, codigo_operadora: e.target.value})} className="w-full bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm font-mono" /></div>
                        <div><label className="block text-sm font-medium mb-1">Nome do Contratado</label><input type="text" value={formData.nome_contratado} onChange={e => setFormData({...formData, nome_contratado: e.target.value})} className="w-full bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm" /></div>
                        <div className="md:col-span-2"><label className="block text-sm font-medium mb-1">Buscar Profissional</label>
                          <div className="flex gap-2"><div className="flex-1 relative"><MagnifyingGlassIcon className="w-4 h-4 absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" /><input type="text" placeholder="Digite o nome do profissional..." value={buscaProfissional} onChange={e => { setBuscaProfissional(e.target.value); buscarProfissionalPorNome(e.target.value); }} className="w-full bg-white dark:bg-gray-700 border rounded-lg pl-8 pr-3 py-2 text-sm" /></div><div className="w-32"><input type="text" placeholder="Nº Conselho" value={buscaProfissional} onChange={e => { setBuscaProfissional(e.target.value); buscarProfissionalPorConselho(e.target.value); }} className="w-full bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm font-mono" /></div>{buscandoProfissional && <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-blue-600"></div>}</div>
                        </div>
                        <div><label className="block text-sm font-medium mb-1">Nome do Profissional</label><input type="text" value={formData.profissional_solicitante} onChange={e => setFormData({...formData, profissional_solicitante: e.target.value})} className="w-full bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm" /></div>
                        <div><label className="block text-sm font-medium mb-1">Conselho</label><select value={formData.conselho_solicitante} onChange={e => setFormData({...formData, conselho_solicitante: e.target.value})} className="w-full bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm">{CONSELHOS.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}</select></div>
                        <div><label className="block text-sm font-medium mb-1">Nº Conselho</label><input type="text" value={formData.numero_conselho_solicitante} onChange={e => setFormData({...formData, numero_conselho_solicitante: e.target.value})} className="w-full bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm font-mono" /></div>
                        <div><label className="block text-sm font-medium mb-1">UF Conselho</label><select value={formData.uf_solicitante} onChange={e => setFormData({...formData, uf_solicitante: e.target.value})} className="w-full bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm">{UF_OPCOES.map(u => <option key={u.value} value={u.value}>{u.label}</option>)}</select></div>
                        <div><label className="block text-sm font-medium mb-1">CBOS</label><input type="text" value={formData.cbos_solicitante} onChange={e => setFormData({...formData, cbos_solicitante: e.target.value})} className="w-full bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm font-mono" /></div>
                      </div>
                    </div>
                  )}
                  ...
                  {aba === 'atendimento' && (
                    <div className="space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div><label className="block text-sm font-medium mb-1">Caráter do Atendimento *</label><select value={formData.carater_atendimento} onChange={e => setFormData({...formData, carater_atendimento: e.target.value})} className="w-full bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm">{CARATER_ATENDIMENTO.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}</select></div>
                        <div><label className="block text-sm font-medium mb-1">Data da Solicitação *</label><input type="date" value={formData.data_solicitacao} onChange={e => setFormData({...formData, data_solicitacao: e.target.value})} className="w-full bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm" required /></div>
                        <div><label className="block text-sm font-medium mb-1">Atendimento a RN</label><select value={formData.atendimento_rn} onChange={e => setFormData({...formData, atendimento_rn: e.target.value})} className="w-full bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm">{SIM_NAO.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}</select></div>
                        <div><label className="block text-sm font-medium mb-1">Tipo de Atendimento *</label><select value={formData.tipo_atendimento} onChange={e => setFormData({...formData, tipo_atendimento: e.target.value})} className="w-full bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm">{TIPO_ATENDIMENTO.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}</select></div>
                        <div><label className="block text-sm font-medium mb-1">Indicação de Acidente</label><select value={formData.indicacao_acidente} onChange={e => setFormData({...formData, indicacao_acidente: e.target.value})} className="w-full bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm">{INDICADOR_ACIDENTE.map(i => <option key={i.value} value={i.value}>{i.label}</option>)}</select></div>
                        <div><label className="block text-sm font-medium mb-1">Tipo de Consulta</label><select value={formData.tipo_consulta} onChange={e => setFormData({...formData, tipo_consulta: e.target.value})} className="w-full bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm">{TIPO_CONSULTA.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}</select></div>
                        <div><label className="block text-sm font-medium mb-1">Regime de Atendimento *</label><select value={formData.regime_atendimento} onChange={e => setFormData({...formData, regime_atendimento: e.target.value})} className="w-full bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm">{REGIME_ATENDIMENTO.map(r => <option key={r.value} value={r.value}>{r.label}</option>)}</select></div>
                        <div><label className="block text-sm font-medium mb-1">Cobertura Especial</label><select value={formData.cobertura_especial} onChange={e => setFormData({...formData, cobertura_especial: e.target.value})} className="w-full bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm">{COBERTURA_ESPECIAL.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}</select></div>
                        <div><label className="block text-sm font-medium mb-1">Saúde Ocupacional</label><select value={formData.saude_ocupacional} onChange={e => setFormData({...formData, saude_ocupacional: e.target.value})} className="w-full bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm">{SAUDE_OCUPACIONAL.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}
                                                  </div>
                        <div><label className="block text-sm font-medium mb-1">Motivo de Encerramento</label><select value={formData.motivo_encerramento} onChange={e => setFormData({...formData, motivo_encerramento: e.target.value})} className="w-full bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm">{MOTIVO_ENCERRAMENTO.map(m => <option key={m.value} value={m.value}>{m.label}</option>)}</select></div>
                      </div>
                      <div><label className="block text-sm font-medium mb-1">Indicação Clínica</label><textarea rows="3" value={formData.indicacao_clinica} onChange={e => setFormData({...formData, indicacao_clinica: e.target.value})} className="w-full bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm" placeholder="Descrição da indicação clínica..." /></div>
                    </div>
                  )}
                  ...
                  {aba === 'procedimentos' && (
                    <div className="space-y-4">
                      <div className="bg-blue-50 p-4 rounded-xl"><p className="text-sm text-blue-700"><strong>ℹ️ Informações:</strong> Visualize os procedimentos que serão executados nesta guia. As horas e profissionais já foram definidos na autorização.</p></div>
                          ...
                          <div className="overflow-x-auto max-h-64">
                            <table className="w-full text-sm">
                              <thead className="bg-gray-50 sticky top-0">
                                <tr>
                                  <th className="px-2 py-2 text-left text-xs font-medium text-gray-500">Seq</th>
                                  <th className="px-2 py-2 text-left text-xs font-medium text-gray-500">Data</th>
                                  <th className="px-2 py-2 text-left text-xs font-medium text-gray-500">H.I</th>
                                  <th className="px-2 py-2 text-left text-xs font-medium text-gray-500">H.F</th>
                                  <th className="px-2 py-2 text-left text-xs font-medium text-gray-500">Código</th>
                                  <th className="px-2 py-2 text-left text-xs font-medium text-gray-500">Procedimento</th>
                                  <th className="px-2 py-2 text-center text-xs font-medium text-gray-500">Qtd</th>
                                  <th className="px-2 py-2 text-right text-xs font-medium text-gray-500">Valor Unit.</th>
                                  <th className="px-2 py-2 text-right text-xs font-medium text-gray-500">Valor Total</th>
                                  <th className="px-2 py-2 text-left text-xs font-medium text-gray-500">Profissional</th>
                                  <th className="px-2 py-2 text-center text-xs font-medium text-gray-500 w-16">Ações</th>
                                </tr>
                              </thead>
                      ...
                      {editandoItem && (
                        <div className="border rounded-xl p-4 bg-blue-50">
                          <div className="flex justify-between items-center mb-3">
                            <h4 className="text-sm font-semibold text-blue-700">Editando Item</h4>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-12 gap-2">
                            ...
                            <div className="md:col-span-1">
                              <button type="button" onClick={() => {
                                ...
                                const itemAtualizado = { ...currentItem, valor_total: valorTotal };
                                setItensGuia(itensGuia.map(item => 
                                  item.id === editandoItem.id ? { ...itemAtualizado, id: item.id } : item
                                ));
                                setEditandoItem(null);
                                toast.success('Item atualizado com sucesso!');
                              }} className="w-full bg-green-600 text-white px-2 py-1.5 rounded-lg text-sm hover:bg-green-700">
                                Atualizar
                              </button>
                            </div>
                          </div>
