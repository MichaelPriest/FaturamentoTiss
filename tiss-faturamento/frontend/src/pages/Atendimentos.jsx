...
import { format, addDays, subDays, startOfWeek, endOfWeek, startOfMonth, endOfMonth, isSameDay, addWeeks, subWeeks, addMonths, subMonths, isWithinInterval } from 'date-fns';
...
export default function Agendamentos() {
  ...
  // Verificar se pode atender (não pode se já foi realizado)
    ...
    <div className="bg-gray-50 dark:bg-gray-900 min-h-screen p-6">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
          <div>
            <h2 className="text-2xl font-bold bg-gradient-to-r from-gray-800 to-gray-600 dark:from-white dark:to-gray-300 bg-clip-text text-transparent">Agendamentos</h2>
            <p className="text-xs text-gray-500 dark:text-gray-400">Gerenciamento de consultas, exames e procedimentos</p>
          </div>
          <div className="flex gap-2">
            <div className="flex bg-gray-100 dark:bg-gray-700 rounded-xl p-1">
              <button onClick={() => setViewMode('dia')} className={`px-3 py-1.5 rounded-lg text-sm font-medium flex items-center gap-1 ${viewMode === 'dia' ? 'bg-white dark:bg-gray-600 shadow-md text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-400'}`}>
                <ListBulletIcon className="w-4 h-4" /> Dia
              </button>
              <button onClick={() => setViewMode('semana')} className={`px-3 py-1.5 rounded-lg text-sm font-medium flex items-center gap-1 ${viewMode === 'semana' ? 'bg-white dark:bg-gray-600 shadow-md text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-400'}`}>
                <ViewColumnsIcon className="w-4 h-4" /> Semana
              </button>
              <button onClick={() => setViewMode('mes')} className={`px-3 py-1.5 rounded-lg text-sm font-medium flex items-center gap-1 ${viewMode === 'mes' ? 'bg-white dark:bg-gray-600 shadow-md text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-400'}`}>
                <CalendarDaysIcon className="w-4 h-4" /> Mês
              </button>
            </div>
            <button onClick={() => { setEditing(null); resetModal(); setShowModal(true); }} className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white px-4 py-2 rounded-xl text-sm flex items-center gap-2 shadow-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-white dark:bg-gray-800 rounded-xl border p-4">
            <div className="flex justify-between"><div><p className="text-xs text-gray-500">Agendamentos Hoje</p><p className="text-2xl font-bold text-blue-600">{agendamentosHoje}</p></div><CalendarIcon className="w-8 h-8 text-blue-500 opacity-50" /></div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-xl border p-4">
            <div className="flex justify-between"><div><p className="text-xs text-gray-500">Esta Semana</p><p className="text-2xl font-bold text-green-600">{agendamentosSemana}</p></div><ClockIcon className="w-8 h-8 text-green-500 opacity-50" /></div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-xl border p-4">
            <div className="flex justify-between"><div><p className="text-xs text-gray-500">Total</p><p className="text-2xl font-bold text-purple-600">{agendamentos.length}</p></div><UserGroupIcon className="w-8 h-8 text-purple-500 opacity-50" /></div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-xl border p-4">
            <div className="flex justify-between"><div><p className="text-xs text-gray-500">Pendentes</p><p className="text-2xl font-bold text-yellow-600">{agendamentosPendentes}</p></div><BellIcon className="w-8 h-8 text-yellow-500 opacity-50" /></div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border p-4 mb-4">
          <div className="flex justify-between items-center flex-wrap gap-4">
            <div className="flex gap-2">
              <button onClick={navegarAnterior} className="p-2 rounded-lg hover:bg-gray-100"><ChevronLeftIcon className="w-5 h-5" /></button>
              <button onClick={navegarProximo} className="p-2 rounded-lg hover:bg-gray-100"><ChevronRightIcon className="w-5 h-5" /></button>
              <button onClick={irParaHoje} className="px-3 py-2 bg-gray-100 rounded-lg text-sm">Hoje</button>
            </div>
            ...
            <div className="flex gap-2">
              ...
              <div className="relative">
                ...
                <input type="text" placeholder="Buscar..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-48 bg-white dark:bg-gray-700 border rounded-lg pl-8 pr-3 py-2 text-sm" />
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Visualização Dia */}
        {viewMode === 'dia' && (
          <div className="bg-white dark:bg-gray-800 rounded-xl border overflow-hidden p-4">
            <div className="text-center py-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg mb-4">
              <h3 className="text-xl font-semibold">{format(currentDate, "EEEE, dd 'de' MMMM 'de' yyyy", { locale: ptBR })}</h3>
            </div>
            <div className="space-y-2">
              {HORARIOS.map(hora => {
                ...
                return (
                  <div key={hora} className={`border rounded-lg p-3 ${agendamento ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-200' : 'bg-white dark:bg-gray-800 border-gray-200 hover:bg-gray-50'}`}>
                      ...
                      {agendamento && (
                        <div className="flex gap-1">
                          {podeAtenderAgendamento ? (
                            ...
                            <button className="p-1 rounded-lg text-gray-400 cursor-not-allowed" title="Atendimento já realizado" disabled>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div><label className="block text-sm font-medium mb-1">Tipo *</label>
                      <select value={formData.tipo} onChange={e => setFormData({...formData, tipo: e.target.value})} className="w-full bg-white dark:bg-gray-700 border rounded-lg px-3 py-2">
                    ...
                    <div><label className="block text-sm font-medium mb-1">Modalidade *</label>
                      <select value={formData.modalidade} onChange={e => setFormData({...formData, modalidade: e.target.value})} className="w-full bg-white dark:bg-gray-700 border rounded-lg px-3 py-2">
