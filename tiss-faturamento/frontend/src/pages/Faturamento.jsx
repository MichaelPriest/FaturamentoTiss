...
export default function Faturamento() {
  ...
  const carregarBloqueados = async () => {
    try {
      ...
      if (data?.valor) {
        setBloqueados(JSON.parse(data.valor));
      }
    } catch (error) {
  ...
  const imprimirRelacao = () => {
    ...
    Object.entries(previewData.conveniosAgrupados).forEach(([convenioId, data]) => {
      conteudo += `
        <h3>Convênio: ${data.convenio?.razao_social}</h3>
        <table>
          <thead>
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="space-y-4">
        {Object.entries(pendentesPorConvenio).map(([convenioId, convenioAtendimentos]) => {
          ...
          return (
            <div key={convenioId} className="bg-white dark:bg-gray-800 rounded-xl border overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                    ...
                    {convenioAtendimentos.map((a) => (
                      <tr key={a.id} className={`hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors ${bloqueados.includes(a.id) ? 'bg-orange-50 dark:bg-orange-900/20' : ''}`}>
                        <td className="px-4 py-3">
                          <input type="checkbox" checked={selecionados.includes(a.id)} onChange={() => handleSelectItem(a.id)} disabled={bloqueados.includes(a.id)} className="rounded w-4 h-4 text-blue-600 focus:ring-blue-500 disabled:opacity-50" />
                        </td>
