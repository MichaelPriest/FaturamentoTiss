...
drop function if exists public.checar_administracao_hospitalar(uuid,timestamptz,text,bigint,text);
drop function if exists public.ativar_prescricao_hospitalar(uuid);

create table if not exists public.prescricoes_hospitalares (
...
create table if not exists public.prescricao_hospitalar_itens (
  id uuid primary key default gen_random_uuid(), empresa_id uuid, unidade_id uuid not null,
  prescricao_id uuid not null references public.prescricoes_hospitalares(id) on delete cascade,
...
create table if not exists public.administracoes_hospitalares (
  id uuid primary key default gen_random_uuid(), empresa_id uuid, unidade_id uuid not null,
  prescricao_item_id uuid not null references public.prescricao_hospitalar_itens(id) on delete restrict,
...
create index if not exists prescricoes_hospitalares_internacao_idx on public.prescricoes_hospitalares(internacao_id,data_prescricao desc);
create index if not exists prescricao_hospitalar_itens_prescricao_idx on public.prescricao_hospitalar_itens(prescricao_id,status);
create index if not exists administracoes_hospitalares_horario_idx on public.administracoes_hospitalares(unidade_id,horario_previsto desc);

do $$ declare tabela text; begin
  foreach tabela in array array['prescricoes_hospitalares','prescricao_hospitalar_itens','administracoes_hospitalares'] loop
...
create or replace function public.ativar_prescricao_hospitalar(p_prescricao_id uuid)
returns public.prescricoes_hospitalares language plpgsql security invoker as $$
declare resultado public.prescricoes_hospitalares; begin
  if not exists(select 1 from public.prescricao_hospitalar_itens where prescricao_id=p_prescricao_id and status='ativo') then
    raise exception 'Inclua ao menos um item antes de ativar a prescrição';
  end if;
  update public.prescricoes_hospitalares set status='encerrada',updated_at=now()
   where internacao_id=(select internacao_id from public.prescricoes_hospitalares where id=p_prescricao_id) and status='ativa' and id<>p_prescricao_id;
  update public.prescricoes_hospitalares set status='ativa',updated_at=now() where id=p_prescricao_id and status='rascunho' returning * into resultado;
  if resultado.id is null then raise exception 'Prescrição em rascunho não encontrada'; end if;
  return resultado;
end $$;

create or replace function public.checar_administracao_hospitalar(
...
) returns public.administracoes_hospitalares language plpgsql security invoker as $$
declare item public.prescricao_hospitalar_itens; resultado public.administracoes_hospitalares; begin
  if p_status not in ('administrado','recusado','nao_administrado','adiado') then raise exception 'Status de checagem inválido'; end if;
  select pi.* into item from public.prescricao_hospitalar_itens pi
    join public.prescricoes_hospitalares p on p.id=pi.prescricao_id
  ...
  insert into public.administracoes_hospitalares(unidade_id,prescricao_item_id,horario_previsto,horario_realizado,status,profissional_id,observacao)
...
grant select,insert,update,delete on public.prescricoes_hospitalares,public.prescricao_hospitalar_itens,public.administracoes_hospitalares to authenticated;
grant execute on function public.ativar_prescricao_hospitalar(uuid) to authenticated;
grant execute on function public.checar_administracao_hospitalar(uuid,timestamptz,text,bigint,text) to authenticated;
commit;
