  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <div className="workspace">
        ...
        {active === 'Recepção e agenda' ? <ReceptionModule onPatientSelected={patient=>setSelected({initials:patient.nome.split(' ').slice(0,2).map(x=>x[0]).join(''),name:patient.nome,id:String(patient.id).padStart(8,'0'),age:'Cadastro ativo',payer:'Consulte o atendimento',journey:'Recepção'})}/> : active === 'Pronto atendimento' ? <TriageModule onPatientSelected={patient=>setSelected({initials:patient.nome.split(' ').slice(0,2).map(x=>x[0]).join(''),name:patient.nome,id:String(patient.id).padStart(8,'0'),age:'Cadastro ativo',payer:'Atendimento atual',journey:'Triagem'})}/> : active === 'Estação clínica' ? <ClinicalModule onPatientSelected={patient=>setSelected({initials:patient.nome.split(' ').slice(0,2).map(x=>x[0]).join(''),name:patient.nome,id:String(patient.id).padStart(8,'0'),age:'Cadastro ativo',payer:'Atendimento atual',journey:'Em atendimento'})}/> : active === 'Farmácia e estoque' ? <PharmacyModule/> : active === 'Visão operacional' ? <><section className="metrics">{liveCards.map(c=><article className={`metric ${c.tone}`} key={c.label}><div><span>{c.label}</span><strong>{c.value}</strong><small>{c.note}</small></div><i/></article>)}</section>
